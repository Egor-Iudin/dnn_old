...
class InputProcessor
{
    private:
        ...
        struct Event
        {
            ...
            /// Default constructor
            Event():
                type(-1), weight(-1.), vars(nullptr),sampleNumber(-1)
            {}
            ...
            Event(UInt_t type_, Double_t weight_, vector<Double_t> const &vars_,UInt_t sampleNumber_):
                type(type_), weight(weight_),sampleNumber(sampleNumber_)
            {
                //nVars = vars_.size();
                vars = new Double_t[nVars];
            ...
            Event(UInt_t type_, Double_t weight_, vector<TTreeFormula *> const &vars_,UInt_t sampleNumber_):
                type(type_), weight(weight_),sampleNumber(sampleNumber_)
            {
                //nVars = vars_.size();
                vars = new Double_t[nVars];
