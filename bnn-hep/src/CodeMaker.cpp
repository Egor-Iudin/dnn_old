...
DNNCodeMaker::DNNCodeMaker(Logger &log_, Config const &config_, InputProcessor const &inputProcessor_):
    log(log_), config(config_), inputProcessor(inputProcessor_),     filePy(config.GetPyFileName().c_str())
{
//	std::cout<<"Hello Python CodeMaker!!!!!!! "<<std::endl;
//	getchar();
	filePy << "inputVarNames=[]\n";
	filePy << "bkgFileNames=[]\n";
	filePy << "sgnFileNames=[]\n";
        filePy << "means=[]\n";
        filePy << "sigmas=[]\n";
	filePy << "trainFileName = \""<<inputProcessor.GetTrainFileName()<<"\"\n";
	filePy << "examFileName = \""<<inputProcessor.examFile1.c_str()<<"\"\n";
	for (auto const &var: config.GetVariables())
	{
		
 
                filePy << "inputVarNames.append(\"" << var << "\");\n";
	}     	
	for (Config::Sample const &sample: config.GetSamples())
    	{
//		std::cout<<"Hello Python CodeMaker22222!!!!!!! "<<std::endl;

        	if (sample.type == 0)
            		filePy << "bkgFileNames.append(\"" << sample.fileName << "\");\n";
        	else
            		filePy << "sgnFileNames.append(\"" << sample.fileName << "\");\n";
    	}
 	unsigned transformIndex = 0;
    	std::ostringstream ost;
     	for (auto const &t: inputProcessor.GetTransformations())
    	{
//		std::cout<<"Hello Python CodeMaker3333333!!!!!!! "<<std::endl;

    	    ost << transformIndex;
            t->WriteCode(filePy, ost.str(),true);
            ost.str("");
            ++transformIndex;
        }
        filePy.close();
    ...
    file(config.GetCPPFileName().c_str())
{
    ...
    nets.reserve(config.GetBNNMCMCIterations() );
    
    //for (unsigned i = config.GetBNNMCMCBurnIn() + 1; i <= config.GetBNNMCMCIterations(); ++i)
    for (unsigned i =  1; i <= config.GetBNNMCMCIterations(); ++i)
    {	    
...
//	getchar();
    ...
    {
        ost << transformIndex;
        t->WriteCode(file, ost.str(),false);
        ost.str("");
        ++transformIndex;
    }
