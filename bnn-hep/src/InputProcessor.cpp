...
InputProcessor::InputProcessor(Logger &log_, Config  &config_, bool trianorExam):
...
//                Double_t const weightValue = weight->EvalInstance();
                ...
                if (localTrainingSet.size() == sample.maxTrainEvents)
		{
           ...
           log << info(0) << "Sample \"" << sample.fileName << "\" number of events " << nEntries <<
                 " train number " << nEntriesRead << " ratio " << 100. * float(nEntriesRead)/float(nEntries)  << eom;
